...
왜 하는가
--------
c=1은 "격자 칸끼리 독립"이라는 가정에서 나온 유도값이다. 그 값이 맞는지를
데이터로 확인한다. 예측 편향(평균 예측확률 - 실제 양성률)이 0을 지나는 c가
유도값 1과 얼마나 떨어져 있는지가 이 실험의 답이다.

주의: 이 스윕은 정답 규칙에 크게 좌우된다. NA를 0으로 세던 판에서는 최적 c가
0.62로 나왔지만, 정답지 정의대로 NA를 평가에서 빼면 답이 달라진다.
결과를 인용할 때 어느 규칙으로 돌린 것인지 반드시 같이 적는다.

조건
----
a=1, b=0 고정이라 학습되는 prior 파라미터가 0개다(area_mode="grid").
따라서 A(1,1) · E(0.5,0.5) · G(1,0.5) · H(1,0.75)는 이 격자 위의 한 점과
정확히 같은 모형이고, 그 점들이 같은 정답 규칙으로 돌린 기존 결과와 맞는지로
스윕을 검증할 수 있다.
