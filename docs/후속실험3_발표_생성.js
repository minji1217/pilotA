...
{
  const s = slideBase(false);
  head(s, "04", "조건 설계 ① — 갈래 ②·③", false);
  s.addText("면적 항이 없는 조건이다. prior 식은 z = a·logit(p̄) + b 이고 a ∈ [0.5, 2], b는 아래 범위에서 학습한다.", {
    x: M, y: 1.78, w: 12.1, h: 0.34, isTextBox: true, margin: 0,
    fontFace: KR, fontSize: 12.5, color: INK2 });

  const rows = [
    ["기준","3번 기준","followup1-1-wood","평균","[−2, 2]","0.504","1.989","6314.7"],
    ["기준","6번 기준","followup2-1-wood","최대","[−2, 4]","0.723","3.956","6288.3"],
    ["1","3번 b4","followup3-avg-b4","평균","[−2, 4]","0.727","3.954","6305.5"],
    ["2","3번 b10","followup3-avg-b10","평균","[−2, 10]","1.568","9.523","6301.4"],
    ["3","6번 b10","followup3-lqmax-b10","최대","[−2, 10]","1.583","9.589","6282.5"],
    ["4","3번 자연혼재","(2번 그대로 · 학습 없음)","평균","—","1.568","9.523","6301.4"],
    ["5","6번 자연혼재","(3번 그대로 · 학습 없음)","최대","—","1.583","9.589","6282.5"],
  ];
  const hi = new Set(["1", "2", "3", "4", "5"]);
  const hdr = ["#","이름","브랜치","LQ 집계","b 범위","학습된 a_LS","학습된 b_LS","NLL"];
  s.addTable([
    hdr.map((h, i) => ({ text: h, options: { bold: true, color: INK, fill: { color: BG2 },
      align: i >= 3 ? "right" : "left", fontSize: 10.5 } })),
    ...rows.map(r => r.map((c, i) => ({
      text: c,
      options: { align: i >= 3 ? "right" : "left", fontFace: i >= 4 ? NUM : KR,
                 bold: hi.has(r[0]), color: hi.has(r[0]) ? INK : INK3,
                 fill: { color: hi.has(r[0]) ? BG : "FAFCFD" } },
    }))),
  ], {
    x: M, y: 2.2, w: 12.1, colW: [0.62, 1.85, 3.0, 1.0, 1.25, 1.42, 1.42, 1.54],
    rowH: 0.34, fontFace: KR, fontSize: 10.5, valign: "middle",
    border: { type: "solid", color: "E3E9EC", pt: 1 },
  });

  note(s, M, 5.05, 5.9, 1.9, "② b는 넓혀도 상한에 붙는다",
    "b_LS가 상한 4에서 3.954, 상한 10에서 9.523으로 둘 다 끝에 붙었다.\n" +
    "최적값을 찾은 것이 아니라 잘린 것이다. 3번과 6번의 차이는 LQ prior를 시정촌 안 " +
    "격자들의 평균으로 집계하느냐 최대로 하느냐 하나뿐이다.", WARN);
  note(s, 6.82, 5.05, 5.9, 1.9, "③ 은 학습을 다시 하지 않는다",
    "4·5번은 2·3번 모델 그대로다. 학습은 GT를 전혀 쓰지 않으므로 같은 seed면 모델이 완전히 " +
    "같고, 바뀌는 것은 LS 정답뿐이다.\n" +
    "자연·혼재 → 양성 / 인공 → 음성(평가 포함) / 불명 → 평가 제외.  418행 → 398행.", GOOD);
}

/* ══════════ 4-4. 조건 설계 ② — 면적 항 A~J ══════════ */
{
  const s = slideBase(false);
  head(s, "04", "조건 설계 ② — 갈래 ④ 면적 항 A~J", false);
  s.addText("z = a · log p̄ + b + c · (log k − m).  면적 항이 있는 조건만 logit이 아니라 log를 쓴다 — λ = p̄·k 를 덧셈으로 넣어야 하기 때문이다.  굵은 값이 학습된 값이다.", {
    x: M, y: 1.78, w: 12.1, h: 0.56, isTextBox: true, margin: 0,
    fontFace: KR, fontSize: 12.5, color: INK2 });

  const rows = [
    ["A","fixed","1.00","0.00","1.00","1.00","0.00","1.00","✗","[−2, 4]","0","유도식 그대로 (기준)"],
    ["B","tied","0.89","4.91","= a","0.50","0.80","= a","✓","[−20, 20]","4","a와 c를 묶으면"],
    ["C","bounded","1.98","11.06","1.00","0.51","0.83","1.00","✓","[−20, 20]","4","c는 1, a·b만 학습"],
    ["D","free","1.86","11.49","0.01","0.50","0.91","0.78","✓","[−20, 20]","6","전부 자유"],
    ["E","a1-c05","1.00","0.00","0.50","1.00","0.00","0.50","✗","[−2, 4]","0","면적 효과 절반"],
    ["F","a1-c2","1.00","0.00","2.00","1.00","0.00","2.00","✗","[−2, 4]","0","면적 효과 두 배"],
    ["G","lq-c05","1.00","0.00","1.00","1.00","0.00","0.50","✗","[−2, 4]","0","LQ만 절반"],
    ["H","lq-c075","1.00","0.00","1.00","1.00","0.00","0.75","✗","[−2, 4]","0","LQ만 0.75"],
    ["I","lq-c-free","1.00","0.00","1.00","1.00","0.00","0.43","✗","[−2, 4]","1","LQ의 c만 학습"],
    ["J","b-only","1.00","5.53","1.00","1.00","2.69","1.00","✓","[−20, 20]","2","높이만 보정"],
  ];
  const learned = new Set(["B", "C", "D", "I", "J"]);
  const hdr = ["","mode","a_LS","b_LS","c_LS","a_LQ","b_LQ","c_LQ","중심화","b 범위","학습","묻는 것"];
  s.addTable([
    hdr.map((h, i) => ({ text: h, options: { bold: true, color: INK, fill: { color: BG2 },
      align: i >= 2 && i <= 10 ? "right" : "left", fontSize: 10 } })),
    ...rows.map(r => r.map((c, i) => ({
      text: c,
      options: { align: i >= 2 && i <= 10 ? "right" : "left",
                 fontFace: i >= 2 && i <= 8 ? NUM : KR,
                 bold: i === 0 || (learned.has(r[0]) && i >= 2 && i <= 7 && c !== "1.00" && c !== "0.00"),
                 color: INK2, fill: { color: BG } },
    }))),
  ], {
    x: M, y: 2.42, w: 12.1, colW: [0.42, 1.28, 0.82, 0.92, 0.82, 0.82, 0.82, 0.82, 0.82, 1.18, 0.66, 2.72],
    rowH: 0.28, fontFace: KR, fontSize: 10, valign: "middle",
    border: { type: "solid", color: "E3E9EC", pt: 1 },
  });

  note(s, M, 5.52, 5.9, 1.52, "중심화는 이름이 아니라 규칙이다",
    "m은 log k 평균(LS 8.042 / LQ 6.656)이다. b에 흡수되는 재매개변수화라 b를 학습하는 " +
    "조건에서만 켠다. b=0 고정 조건은 빼면 유도식이 깨진다.", null);
  note(s, 6.82, 5.52, 5.9, 1.52, "자기 prior 순위는 c/a 하나로 정해진다",
    "b와 m은 전 행에 더해지는 상수라 순위를 못 바꾸고, a와 c가 같은 배율이면 스케일만 변한다. " +
    "그래서 A·B·G·H·I·J의 prior AUC가 0.8850으로 전부 같다.", WARN);
}
