...
PRESETS = {
    ...
    "followup3-avg-b4": {"modes": ("bounded",), "lams": (10.0,),
                         "b_min": -2.0, "b_max": 4.0,
                         "tag_suffix": "_b4"},
    "followup3-avg-b10": {"modes": ("bounded",), "lams": (10.0,),
...
DEFAULT_PRESET = "followup3-avg-b10"
...
def run_one(batch, eval_gt, gt_df, *, mode, lam, epochs, seed, b_bound=DEFAULT_B_BOUND,
    ...
    result = evaluate(gt_df, to_eval_pred(batch, p_ls, p_lq, eval_gt, pri=pri))
